...
For MSE, we observe that the difference is positive and increasing in $\sigma^2_u$. The difference for $\hat{\sigma^2_\epsilon}_{MME}$ is greater than that of $\hat{\sigma^2_\epsilon}_{OLS}$ for the same values of $\sigma^2_u$ and $\sigma^2_\epsilon$.

\subsubsection{$\chi^2$-distributed observation error}

\section{Conclusion}

This report presents some epirical results on the case of non-normality under the errors-in-variables model. 

\subsection{Further work}

Exact inference

Exist better estimators
